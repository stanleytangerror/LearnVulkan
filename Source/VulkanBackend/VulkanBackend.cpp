...
void createDescriptorSets(VkDevice device, std::vector<VkDescriptorSet>& descriptorSets, 
...
void createFramebuffers(VkDevice device, 
	const std::vector<VkImageView>& swapChainImageViews, std::vector<VkFramebuffer>& swapChainFramebuffers,
...
void createCommandPool(VkPhysicalDevice physicalDevice, VkDevice device, const VulkanBackend::QueueFamilyIndices& queueFamilyIndices, VkCommandPool& commandPool) {
...
//void recreateSwapChain(VkPhysicalDevice physicalDevice, VkDevice device, Platform::IWindow* window,
...
void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex, 
	...
	uint32_t currentFrame, VkRenderPass renderPass,
...
void updateUniformBuffer(const std::vector<void*>& uniformBuffersMapped, uint32_t currentImage, VkExtent2D swapChainExtent) {
	...
	const std::vector<VkDescriptorSet>& descriptorSets)
{
	...
	recordCommandBuffer(commandBuffers[currentFrame], imageIndex, swapChainFramebuffers, swapChainExtent, 
		graphicsPipeline, pipelineLayout, vertexBuffer, indexBuffer, currentFrame, renderPass, descriptorSets);
...
private:
	std::unique_ptr<VulkanBackend::VulkanGraphicsInfra> mInfra;
	...
	void initWindow() {
		window = CreateNativeWindow(L"Vulkan", Vec2u{ WIDTH, HEIGHT });
	}
	...
	void mainLoop() {
		mInfra = std::make_unique<VulkanBackend::VulkanGraphicsInfra>();
		mInfra->AdaptToWindow(window);
		...
		auto graphicsQueue = mInfra->GetGraphicsQueue();
		auto presentQueue = mInfra->GetPresentQueue();
		...
		createFramebuffers(device, swapChainImageViews, swapChainFramebuffers, swapchainExtent, renderPass);
		createCommandPool(physicalDevice, device, queueFamilyIndices, commandPool);
